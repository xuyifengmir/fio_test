...
do
    if echo "${disk}" |grep nvme > /dev/null;then
        ...
        mkfs.xfs /dev/${disk}
        mount -o discard /dev/${disk} /data/${disk}
...
do
    ...
    sed -i "s/[0-9]\{1,\}[k|K]/${bss}k/g" baseline_xfs_tlc.fio
    fio ${comp_opt_str} \
     --directory=/data/${disk} \
     --filename=${disk}.fio \
     --output=${timestamp}/${disk}_ext4_file.fio \
     ./baseline_xfs_tlc.fio &
    pid_list="${pid_list} $!"
done
