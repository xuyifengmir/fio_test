...
do
    ...
    elif echo "${disk}" |grep sfd > /dev/null;then
        echo y | sfx-nvme format "/dev/""$disk"
        sfx-nvme smart-log /dev/${disk} > ${timestamp}/${disk}_smart-log_start.txt
        sfx-nvme sfx smart-log-add /dev/${disk} > ${timestamp}/${disk}_smart-log-add_start.txt
        sfx-status  /dev/${disk}  > ${timestamp}/${disk}_sfx-status_start.txt
    fi
done
...
do
    if echo "${disk}" |grep nvme > /dev/null;then
        ...
        sfx-nvme smart-log /dev/${disk} > ${timestamp}/${disk}_smart-log_end.txt
        sfx-nvme sfx smart-log-add /dev/${disk} > ${timestamp}/${disk}_smart-log-add_end.txt
        sfx-status  /dev/${disk}  > ${timestamp}/${disk}_sfx-status_end.txt
    fi
done
